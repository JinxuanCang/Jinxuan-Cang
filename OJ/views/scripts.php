  ...
  <script type="text/javascript"> 
    ...
    function Clear() {
      if(editor.getValue()=="" || editor.getValue()==null) {
        LoadMessage("Notice","Textarea cleared already.")
      }
      else {
        Confirm('null','clear');
      }
	  ...
	  function Duplicate(file_name) {
		  ...
		  for (var i = 0; filter_name[i]!="stop"; i++) {
            ...
            if (filter_name[i]==r_name+"_copied"+e_name) {
			        ...
			        var sensor = 1;
			        LoadMessage("Caution","<b>"+r_name+"_copied"+e_name+"</b> already exist.");
              break;
            }
      }
		  if (sensor==0) {
          ...
          <?php
            if (isset($_GET["file_name"])) {
              ...
              if ($_GET["action"]=="edit") {
                ...
                if ($file_connect) {
                  echo "<script>LoadMessage(\"Okay\",\"Successfully connected to <b>$file_name</b>.\")</script>";
                }
                else {
                  echo "<script>LoadMessage(\"Error\",\"<b>$file_name</b> doesn't exist.\")</script>";
                }
              }
              if ($_GET["action"]=="duplicate") {
					      if(copy($path.$file_name, $path.$r_name."_copied".$e_name)) {
						      echo "<script>LoadMessage(\"Okay\",\"Successfully duplicated <b>$file_name</b>.\")</script>";
					      }
					      else {
						      echo "<script>LoadMessage(\"Error\",\"Failed to duplicate <b>$file_name</b>.\")</script>";
					      }
              }
              if ($_GET["action"]=="rename") {
                if (rename($path.$file_name, $path.$n_name)) {
                  echo "<script>LoadMessage(\"Okay\",\"Successfully renamed <b>$file_name</b> to <b>$n_name</b>.\")</script>";
                }
                else {
                  echo "<script>LoadMessage(\"Error\",\"Failed to rename <b>$file_name</b>.\")</script>";
                }
              }
            ...
            if ($_GET["file_name"]!="" and $_GET["file_name"]!=null and $_GET["action"]!="" and $_GET["action"]!=null) {
              ...
              if ($action=="delete") {
                if(unlink($path.$file_name)) {
                  echo "<script>LoadMessage('Okay','Successfully deleted <b>$file_name</b>.')</script>";
                }
                else {
                  echo "<script>LoadMessage('Error','Failed to delete <b>$file_name</b>.')</script>";
                }
              }
              if ($action=="edit") {
                $editor = 1;
        ...
        <?php
          if (isset($_POST["data_name"]) and isset($_POST["data_type"]) and isset($_POST["data_value"])) {
            ...
            if(!unlink($path.$f_name)) {
              $message = 1;
              echo ("<script>LoadMessage('Error','Failed to refresh <b>$f_name</b>.')</script>");
              goto close;
            }
            ...
            if(fwrite($value, $f_value)) {
              $message = 1;
              echo "<script>LoadMessage('Okay','Successfully modified <b>$f_name</b>.')</script>";
            }
            else {
              $message = 1;
              echo ("<script>LoadMessage('Okay','Set <b>$f_name</b> to empty.')</script>");
            }
            close:
            fclose($value);
          }
        ...
        <?php
           ...
           while (array_key_exists($i, $file)) {
             ...
             echo "\r\n\t\t\t";
           	 ...
           	 echo "<td>\r\n\t\t\t\t<a onclick='Loading();' href='scripts.php?file_name=$file[$i]&action=edit'><img src='../css/img/b_edit.png'>Edit</a>\r\n\t\t\t\t";
             echo "<a onclick='Loading();' href='javascript:Duplicate(\"$file[$i]\")'><img src='../css/img/b_relations.png'>Duplicate</a>\r\n\t\t\t\t";
             echo "<a href='../scripts/$file[$i]' download><img src='../css/img/b_export.png'>Export</a>\r\n\t\t\t\t";
             echo "<a onclick='Loading();' href='javascript:Confirm(\"$file[$i]\",\"rename\")'><img src='../css/img/b_inline_edit.png'>Rename</a>\r\n\t\t\t\t";
